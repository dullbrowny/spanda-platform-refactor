...
def grade_qa_pair(qa_pair):
    ...
    if response.status_code == 200:
        ...
        raise Exception(
            f"Failed to grade Q&A pair: {response.status_code}, Response: {response.text}"
        )
                    ...
                    for file in filearea["files"]:
                        try:
                            ...
                            for i, qa_pair in enumerate(qa_pairs):
                                try:
                                    justification, avg_score = grade_qa_pair(
                                        qa_pair
                                    )
    ...
    try:
        ...
        activity = next(
            (
                a
                for a in activities
                if a["name"].strip().lower() == activity_name.strip().lower()
            ),
            None,
        )
        if not activity:
        ...
        print(
            f"{activity_type.capitalize()} '{activity_name}' found with ID: {activity_id}"
        )
        ...
        print("\n=== Processing Submissions ===")
        processed_data = [
            process_user_submissions(user, submissions_by_user, activity_type)
            for user in users
        ]
